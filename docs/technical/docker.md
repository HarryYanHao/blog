...
-p 3000:3000：将容器内的 3000 端口映射到主机的 3000 端口，这样可以通过主机访问容器内运行的 Node.js 应用。
-v $(pwd):/app：将当前工作目录（$(pwd)）挂载到容器内的 /app 目录，实现本地代码和容器内代码的同步。
-w /app：设置容器的工作目录为 /app。
node:18：指定使用的 Node.js 镜像版本。
node app.js：在容器内执行的命令，即运行 app.js 文件。
### 开发的测试环境
